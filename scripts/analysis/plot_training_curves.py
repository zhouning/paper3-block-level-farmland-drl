# -*- coding: utf-8 -*-
"""Training curves for Paper 3 Block-Level DRL (Township 109, 5 seeds)."""
...
LOG_DIR = BLOCK_RESULTS_DIR / 'township_500227109'
...
# Smooth function (rolling window)
...
metrics = [
    ...
    ('slope_change_pct', 'Slope Change (%)', None),
    ('cont_change', 'Contiguity Change', None),
    ('baimu_area_change_ha', 'Baimu Area Change (ha)', None),
    ('budget_used', 'Budget Used (swaps)', None),
]

# Baselines for reference lines
baselines_ref = {
    'slope_change_pct': [
        (-2.12, 'Greedy-Seq', '#FF9800'),
        (-6.36, 'Greedy-Global', '#E53935'),
    ],
    'cont_change': [
        (0.050, 'Greedy-Seq', '#FF9800'),
        (0.439, 'Greedy-Global', '#E53935'),
    ],
    'baimu_area_change_ha': [
        (-12.0, 'Greedy-Seq', '#FF9800'),
        (-31.5, 'Greedy-Global', '#E53935'),
        (43.8, 'Round-Robin', '#4CAF50'),
    ],
}
...
# Last subplot: convergence analysis
ax = axes[5]
# Plot reward trajectory with convergence markers
for seed in range(5):
    data = all_data[seed]
    timesteps = [d['timestep'] for d in data]
    rewards = [d['reward'] for d in data]

    # Compute running mean in windows of 500 episodes
    chunk_size = 500
    chunk_ts = []
    chunk_means = []
...
plt.suptitle('Township 109 (A-Small): Training Curves — 200K Steps × 5 Seeds\n'
             'Episode = 20 steps, ~10,000 episodes per seed',
             fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig(FIGURES_DIR / 'paper3_training_curves_109.png', dpi=600, bbox_inches='tight')
plt.close()
print("Saved: paper3_training_curves_109.png")
